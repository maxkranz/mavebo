...
export async function adminAddBadge(userId: string, badgeType: string, password: string) {
  ...
  // Validate badge type
  const validBadges = ["verified", "whale", "early"]
...
export async function adminDeleteUserContent(userId: string, contentType: string, password: string) {
  ...
  try {
    ...
    switch (contentType) {
      case "photos":
        ({ error } = await supabaseAdmin.from("photos").delete().eq("author_id", userId))
        break
      case "collections":
        ({ error } = await supabaseAdmin.from("collections").delete().eq("author_id", userId))
        break
      case "albums":
        ({ error } = await supabaseAdmin.from("albums").delete().eq("author_id", userId))
        break
      case "comments":
        ({ error } = await supabaseAdmin.from("comments").delete().eq("author_id", userId))
        break
      default:
